...
window.toastr = require('./../plugins/toastr/toastr.js');
...
axios.interceptors.request.use(function (config) {
	...
	$('small').parent().removeClass('has-error');
...
axios.interceptors.response.use(function (response) {
	...
	if (errors.status == 401) {
		toastr.info('Verifique su autentificacion');
	}
	if (errors.status == 422) {
		errors = errors.data.errors;
		for(let e in errors) {
			$('small#' + e + 'Help').parent().addClass('has-error');
